...
<script>
	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			editable: true,
			events: "<?php echo U("Schedule/json");?>",//"json-events.php",
			...
			eventDrop: function(event,dayDelta,minuteDelta,allDay,revertFunc) {
				...
				$.ajax({
					url:"<?php echo U('Schedule/drop');?>?id="+event.id+"&start="+start+"&end="+end,
					type:"get",
					dataType:"json",
			...
			dayClick: function(date, allDay, jsEvent, view) {
 				var selDate =$.fullCalendar.formatDate(date,'yyyy-MM-dd');
					$.fancybox({
						'type':'ajax',
						'href':'<?php echo U('Schedule/window');?>?action=add&date='+selDate
					});
			},
			eventClick: function(calEvent, jsEvent, view) {
				$.fancybox({
					'type':'ajax',
					'href':'<?php echo U('Schedule/window');?>?action=edit&id='+calEvent.id
				});
	    	}
		});
